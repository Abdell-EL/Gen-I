...
from app.services.password_reset_service import (
    ConsumedPasswordResetTokenError,
...
SIGNING_KEY = (
    "postgres-reset-completion-test-key-0123456789abcdef0123456789abcdef"
    "0123456789abcdef"
)
SETTINGS = AuthSettings(
    jwt_secret=SIGNING_KEY,
    jwt_issuer="postgres-reset-completion-tests",
    jwt_audience="postgres-reset-completion-client",
...
class PostgresResetCompletionConcurrencyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        self.unique = f"phase5b1a3b-{uuid4().hex}"
        self.email = f"{self.unique}@example.com"
        self.old_password = f"{self.unique}-old-password"
        self.raw_token = f"{self.unique}-reset-token-with-enough-length"
        self.new_passwords = {
        ...
        with self.Session() as db:
            user = User(
                full_name="Phase 5B Reset Completion Concurrency User",
                email=self.email,
                role="agent",
                is_active=True,
            ...
            db.commit()
            db.refresh(token)
            self.reset_token_id = token.password_reset_token_id
    ...
    def _run_concurrent_completion(self) -> tuple[list[dict[str, object]], bool]:
        if self.user_id is None:
            raise AssertionError("Test user was not created.")
        ...
        original_hash_password = password_reset_service.hash_password
        ...
        def worker(label: str, new_password: str) -> None:
            ...
            except ConsumedPasswordResetTokenError:
                outcomes.put(
                    {
                        "label": label,
                        "category": "consumed",
        ...
        with patch.object(
            ...
            threads = [
                Thread(target=worker, args=(label, new_password), daemon=True)
        ...
        self.assertEqual(categories.count("consumed"), 1)
        loser = next(result for result in results if result["category"] == "consumed")
    ...
    def test_concurrent_reset_completion_locks_token_row(self):
        pre_reset_jwt = self._token_for_current_user(version=0)

        results, real_blocking_observed = self._run_concurrent_completion()
        ...
        with self.Session() as db:
            ...
            token = db.get(PasswordResetToken, self.reset_token_id)
            ...
            forbidden_values = (
                ...
                user.password_hash,
                pre_reset_jwt,
                f"https://example.com/reset?token={self.raw_token}",
            )
        ...
        with patch("app.auth_dependencies.get_auth_settings", return_value=SETTINGS):
            client = self._client()
            stale = client.get(
                "/api/v1/auth/me",
                headers={"Authorization": f"Bearer {pre_reset_jwt}"},
            )
