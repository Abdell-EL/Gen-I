...
`POST /api/v1/auth/password/reset/complete` is public and completes reset-token use without signing the user in automatically. The service first applies the same token-length guard as validation, then locates and locks the matching `password_reset_tokens` row with `SELECT ... FOR UPDATE`. After the lock is acquired, token lifecycle is rechecked so duplicate or concurrent use observes the committed consumed/invalidated/expired state. The associated user row is also reloaded with `SELECT ... FOR UPDATE` and must still be active, activated, and have an existing password hash. Ineligible account state maps to the same generic invalid-reset response and does not reveal user state.

Reset completion is the transaction owner. In one transaction it validates password confirmation, applies the authoritative UTF-8 password byte-limit via the existing password helpers, rejects empty passwords and same-password reuse, hashes and replaces `users.password_hash`, increments `users.token_version` exactly once, sets the used token `consumed_at`, invalidates every other outstanding unconsumed and not-yet-invalidated reset token for that user, updates `users.updated_at`, writes one secret-free `password_reset_completed` audit event, and commits once. The used token is consumed but not also invalidated. Consumed tokens, previously invalidated tokens, and other users’ tokens are not rewritten. Any hashing, audit, or database failure rolls back password replacement, token-version increment, token consumption, token invalidation, and audit state.
